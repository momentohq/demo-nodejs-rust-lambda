...
pub fn initialize_logging(log_level: &str) {
    ...
    // this logger is initialized globally for the Rust process. In an
    // AWS Lambda environment, the initialized state can persist
    // across lambda invocations, so we need to handle the case where
...
impl MomentoCacheFacade {
    pub fn new(momento_api_key: String, cache_name: String) -> MomentoCacheFacade {
        MomentoCacheFacade {
            cache_name,
